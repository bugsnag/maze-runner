...
class CompareTest < Test::Unit::TestCase
  ...
  def test_different_classes
    result = Maze.value_compare({}, [])
    assert_false(result.equal?)
    assert_equal('<root>', result.keypath)
  end

  def test_array_value_types
    result = Maze.value_compare(['1'], [1])
    assert_false(result.equal?)
    assert_equal('0', result.keypath)
  end

  def test_array_length
    result = Maze.value_compare(['1'], ['1', 1])
    assert_false(result.equal?)
    assert_equal('<root>', result.keypath)
    assert_true(Maze.value_compare(['1'], ['1']).equal?)
  end

  def test_array_nested_types
    assert_false(Maze.value_compare(['1', { "color": 'grey' }], ['1', { "color": 'red' }]).equal?)
    assert_true(Maze.value_compare(['1', { "color": 'grey' }], ['1', { "color": 'grey' }]).equal?)
  end

  def test_nested_object_key_paths
    template = { "breakdown": [1,2,{ "animal":'bear' }] }
    actual = { "breakdown": [1,2,{ "animal":'fox' }] }
    result = Maze.value_compare(template, actual)
